...
let input_mouse_or_touch _ =
  if true then (is_mouse_button_down MouseButton.Left, get_mouse_position ())
...
let input_playing model =
  ...
  let inp =
    if
      (match model.player.input with Aiming _ -> true | _ -> false)
      && not touched
    then
      ...
      (match model.player.input with Aiming _ -> false | _ -> true)
      && touched
      && Float.pow (tx -. px) 2.0 +. Float.pow (ty -. py) 2.0
         < touch_catch_size *. touch_catch_size
    then Aiming (tx, ty)
    else if
      (match model.player.input with Aiming _ -> true | _ -> false)
      && touched
    then Aiming (tx, ty)
    else if jump then Jump
    else if (not ccw) && cw then CW
    else if ccw && not cw then CCW
    else if touched && touch_x < 2 * ssize then CCW
    else if touched && touch_x > screen_width - (2 * ssize) then CW
    else None
  in
  let new_t = {model with player= {model.player with input= inp}} in
  if paused then Model.Paused new_t else Model.Playing new_t

let input_paused model =
  let unpaused =
    is_key_pressed Key.Space || is_key_pressed Key.P || is_key_pressed Key.A
    || is_key_pressed Key.S
  in
...
let input_menu _ =
  ...
  | _ when touch_area (sf 5.0) (sf 6.5) (sf 6.0) (sf 0.5) ->
      Model.WorldSelect
  | _ when touch_area (sf 5.0) (sf 7.0) (sf 6.0) (sf 0.5) ->
      Model.StatsScreen None
  | _ -> Model.MenuScreen
...
let input_stats p =
  ...
  | Some (_, b) -> (
    match p with
    | None when b <= List.length Level.world_names ->
        Model.StatsScreen (Some (b * 100, None))
    | None -> Model.StatsScreen p
...
let input_levelend p =
  ...
  if is_key_pressed Key.Space then
    Model.LevelSelect (((p.id / 100) + 1) * 100)
...
let input_world _ =
  ...
  let sf x = int_of_float (float_of_int ssize *. x) in
  let touchmap =
    List.init (List.length Level.world_names) (fun i ->
  ...
  | Some (_, b) when b <= List.length Level.world_names ->
