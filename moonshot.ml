...
module Powerup = struct
  ...
  type t = {power: kind; pos: Vector2.t}

  let draw_size = function Fireblast -> 1.0
end

module Body = struct
  type surface_type = Normal | Painful | Sticky | Bouncy | Slippery

  type bulletkind = Fireblast | Normalblast
  ...
  type fading =
    {remaining: float; (* in seconds *)
                       body: t; explosion_kind: bulletkind}

  type planet = {body: t; surface: surface_type}

  type moving = {vel: Vector2.t; (* in m/s *)
                                 body: t}

  type bullet = {kind: bulletkind; created_at: float; moving: moving}
end
...
module Player = struct
  ...
  type animation_action = Standing | Falling | Walking | WalkingReverse
...
module Enemy = struct
  type action = Standing | Shielded | Dead of float

  type t = {loc: Body.moving; action: action; angle: float}

  let is_alive x = match x.action with Dead _ -> false | _ -> true
end

module Model = struct
  type star_requirements = {time: float; shots: int; health: int}

  type endreason = Victory | Died | DriftedAway
...
let vector v = (Vector2.x v, Vector2.y v)

let screen_of_world w = int_of_float (w *. pixels_per_meter)

let world_of_screen p = float_of_int p *. meters_per_pixel

let screen_of_world_vector v =
  let x = (screen_of_world @@ Vector2.x v) + (screen_width / 2) in
  let y =
    screen_height - (screen_of_world @@ Vector2.y v) - 1 - (screen_height / 2)
  in
  (x, y)
...
let sofwv_v v =
  let x, y = screen_of_world_vector v in
