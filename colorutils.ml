...
(** Expects values to be from [0,255] * returns values in range [0-360),
    [0-100), [0-100) *)
let hsv_of_rgb (r, g, b) =
  ...
  in
  let s =
    int_of_float (100.0 *. if cmax = 0.0 then 0.0 else delta /. cmax)
  in
  let v = int_of_float (100.0 *. cmax) in
  (h, s, v)

(** Expects values to be in range [0-360), [0-100), [0-100) * returns values
    from [0,255] *)
