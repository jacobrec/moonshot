...
let get_level_offset i = static_properties + (i * max_properties)

let get_best_time_offset i = get_level_offset i + offset_time

let get_best_health_offset i = get_level_offset i + offset_time

let get_best_shots_offset i = get_level_offset i + offset_time

let has_star_time_offset i = get_level_offset i + offset_time_star

let has_star_health_offset i = get_level_offset i + offset_health_star

let has_star_shots_offset i = get_level_offset i + offset_shots_star

let has_level_completed_offset i =
  get_level_offset i + offset_level_completed

let write_totals a b c d =
  let b = int_of_float (b *. 100.0) in
  (* time is stored to 2 decimals *)
  let c = int_of_float (c *. 10.0) in
  (* health is stored to 1 decimal *)
...
let save levelend =
  ...
  let new_stars =
    ...
    +
    if has_health_star && 0 = load (has_star_health_offset id) then 1 else 0
  in
