...
let accelerate_moving_body delta b (acc_x, acc_y) =
  let open Moonshot.Body in
  let vel_x, vel_y =
    (Vector2.x b.vel +. (acc_x *. delta), Vector2.y b.vel +. (acc_y *. delta))
  in
  let pos_x, pos_y =
    ( Vector2.x b.body.pos +. (vel_x *. delta)
    , Vector2.y b.body.pos +. (vel_y *. delta) )
  in
  let pos = Vector2.create pos_x pos_y in
  let vel = Vector2.create vel_x vel_y in
  {Moonshot.Body.vel; body= {b.body with pos}}
...
let vector_distance v1 v2 = Float.sqrt @@ vector_distance' v1 v2

let angle_from_vectors' (x1, y1) (x2, y2) = Float.atan2 (y2 -. y1) (x2 -. x1)
...
let update_planet_collidable ?(inp = None) ?(slipping = false)
    ?(no_forces = false) delta bodies base =
  let open Moonshot.Body in
  let b = base in
  let b' =
    ...
    | Some planet ->
        ...
        let vpar, vper =
          ...
          | Some inp -> (
            ...
            | Moonshot.Player.Jump ->
                play_sound Moonshot.jump_sound ;
                (vpar_jump -. jump_force, vper)
            | _ -> (vpar, vper) )
        in
        (* Friction *)
        let friction = if slipping then 1.0 else 0.90 in
...
let update_player delta bodies static fading player =
  let { Moonshot.Player.head= float
      ; feet= base
      ; input
      ; health
      ; last_damaged_at
  ...
  let on_planet_type t =
    ...
    List.exists
      (fun b2 ->
        ignore b2.surface ;
        bodies_touch base.body b2.body)
      planets
  in
  ...
  in
  let damaged =
    (in_any_explosion || touching_ouchy_planets)
    && last_damaged_at +. Moonshot.damage_cooldown < get_time ()
  in
  let health = if damaged then health - 1 else health in
  let input =
    if damaged || touching_bouncy_planets then Player.Jump else input
  in
  let last_damaged_at = if damaged then get_time () else last_damaged_at in
  let animation_state =
    if not touching_a_planet then Player.Falling
  ...
  (* Calculate new base *)
  let new_b, (b_fx, b_fy) =
    update_planet_collidable ~slipping:touching_icy_planets ~inp:(Some input)
      delta bodies base
  in
  let new_f = float in
  ...
  Vector2.set_x new_f.body.pos
    (Vector2.x new_b.body.pos +. (seperation *. Float.cos gravity_dir)) ;
  Vector2.set_y new_f.body.pos
    (Vector2.y new_b.body.pos +. (seperation *. Float.sin gravity_dir)) ;
  { player with
    Moonshot.Player.head= new_f
  ; feet= new_b
  ; health
  ; last_damaged_at
  ; animation_state }
...
let create_bullet_from_aim ax ay head =
  ...
  {Body.body= {Body.pos= Vector2.create px py; mass= 1.0; radius= 0.5}; vel}
...
let create_player_bullets special_shots time player =
  ...
  | Moonshot.Player.Fire (ax, ay) ->
      ...
      , [ { Body.created_at= time
          ; kind
          ; moving= create_bullet_from_aim ax ay player.head } ] )
  | _ -> (special_shots, [])
...
let explosion_from_body time x =
  ...
  | Normalblast ->
      ( t
      , { remaining= Moonshot.explosion_time
        ; explosion_kind= kind
        ; body=
            { x.body with
              radius= Moonshot.explosion_radius
            ; mass= Moonshot.explosion_mass } } )
  | Fireblast ->
      ( t
      , { remaining= Moonshot.explosion_time
        ; explosion_kind= kind
        ; body=
            { x.body with
              radius= Moonshot.explosion_radius /. 2.0
...
let update_enemies delta bodies fading enemies =
  ...
  let open Moonshot.Body in
  let in_explosions b1 =
    List.filter
      (fun b2 ->
        ignore b2.remaining ;
        bodies_touch b1.loc.body b2.body)
      fading
  in
  let in_explosion_kind b t =
    let explos = in_explosions b in
    List.exists (fun x -> x.explosion_kind = t) explos
  in
  let on_a_planet b1 =
    List.exists (fun b2 -> bodies_touch b1.loc.body b2) bodies
  in
  let enemies =
    List.map
      (fun x ->
        (* check explosion status *)
        if x.action = Standing && in_explosion_kind x Body.Normalblast then
          {x with action= Dead 2.0}
        else if x.action = Shielded && in_explosion_kind x Body.Fireblast
        then {x with action= Standing}
        else x)
      enemies
  in
  let enemies =
  ...
  let update_enemy e =
    let loc, (fx, fy) =
      if not (on_a_planet e) then update_planet_collidable delta bodies e.loc
      else update_planet_collidable ~no_forces:true delta bodies e.loc
    in
    {e with loc; angle= Float.atan2 fy fx}
  in
...
let update_camera cam player =
  ...
  let new_zoom =
    Camera2D.zoom cam +. Float.copy_sign clamped_abs_diff_zoom diff_zoom
  in
...
let update_playing_check_for_level_end model =
  ...
  let longest_bullet =
    List.fold_left
      (fun a x ->
        let open Body in
        Float.max a (model.runtime -. x.created_at))
      model.longest_bullet bullets
  in
  let end_conditions =
    [ ((fun _ -> model.player.health <= 0), Model.Died)
    ; ( (fun _ ->
          let x, y = vector model.player.feet.body.pos in
          (x *. x) +. (y *. y) > 100.0 *. 100.0
          && not
               (List.exists
                  (fun p ->
...
let update_planets static fading =
  ...
  List.map
    (fun planet ->
      ...
      let fading =
        List.filter (fun x -> x.explosion_kind = Body.Fireblast) fading
      in
      if planet.surface = Slippery then
        if
          List.exists
            (fun b -> bodies_touch (explosion_body b) (planet_body planet))
            fading
        then {planet with surface= Normal}
        else planet
      else planet)
    static
...
let update_playing delta model =
  ...
  let body_set ?(bullets = []) ?(static = []) ?(fading = []) ?(enemies = [])
      _ =
    List.concat
      [ List.map explosion_body fading
  ...
  let fading =
    List.map
      (fun x ->
        let percent = x.remaining /. Moonshot.explosion_time in
        let r = Moonshot.explosion_radius /. 2.0 in
        { x with
          remaining= x.remaining -. delta
        ; body=
            { x.body with
              mass= percent *. Moonshot.explosion_mass
            ; radius= r +. (r *. percent) } })
      fading
  in
  ...
  let movables =
    List.map
      (update_bullet delta static (body_set ~fading ~static ()))
      bullets
  in
  let in_any_static b1 =
    List.exists
      (fun b2 -> bodies_touch b1.moving.body b2)
      (body_set ~fading
         ~static:(List.filter (fun x -> x.surface <> Body.Bouncy) static)
         ~enemies ())
  in
  ...
  let longest_bullet =
    List.fold_left (fun a x -> Float.max a x) longest_bullet bullet_time
  in
  let create_explosions = List.map (fun (_, a) -> a) create_explosions in
  let fading = List.concat [fading; create_explosions] in
  let player =
    update_player delta (body_set ~fading ~static ()) static fading player
  in
  let new_power, powerups = update_powerups player powerups in
  let special_shots = List.concat [special_shots; new_power] in
  let special_shots, player_bullets =
    create_player_bullets special_shots runtime player
  in
  let shots_taken = shots_taken + List.length player_bullets in
  let new_bullets = List.concat [alive; player_bullets] in
  let enemies =
    update_enemies delta (body_set ~fading ~static ()) fading enemies
  in
  let living_enemies =
    List.filter (fun x -> Moonshot.Enemy.is_alive x) enemies
  in
  let cam = update_camera cam player in
  let runtime =
    if 0 = List.length living_enemies then runtime else runtime +. delta
  in
