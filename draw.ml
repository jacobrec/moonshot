...
let draw_aim_assist static dots freq ax ay player =
  ...
  let open Moonshot.Body in
  let bouncy, not_bouncy =
    List.partition (fun x -> x.surface = Body.Bouncy) static
  in
  ...
  in
  let every_few =
    filteri (fun i _ -> i mod (int_of_float (freq /. timing) + 1) = 0) all
  in
  List.iter
    (fun x ->
      let open Moonshot.Body in
      draw_body Color.darkgray x.body)
    every_few
...
let draw_texture text wloc ang size =
  ...
  let p =
    Vector2.create
      (px -. (p_off *. Float.cos ang) +. (p_off *. Float.sin ang))
      (py +. (p_off *. Float.sin ang) +. (p_off *. Float.cos ang))
  in
...
let draw_explosion x =
  ...
  draw_texture
    (Images.get_animation ~time:timein Images.Explosion)
    x.body.pos 0.0 (2.0 *. x.body.radius) ;
...
let draw_enemy e =
  ...
  let c, t =
    ...
    | _ -> (Color.blue, Images.get_animation Images.EnemyStanding)
  in
  let size = 2.0 in
  draw_texture t e.loc.body.pos e.angle size ;
  if Moonshot.debug_draw then draw_body c e.loc.body
...
let draw_player ani pfeet phead =
  ...
  let tex =
    ...
    | Player.WalkingReverse ->
        Images.get_animation Images.PlayerWalkingReverse
    | Player.Falling -> Images.get_animation Images.PlayerFalling
  in
  ...
  if Moonshot.debug_draw then
    ...
    @@ List.map (fun x -> x.body) [phead; pfeet]
...
let draw_blocky_circle x y r ci co =
  ...
  let square x = x * x in
  let lerp a b x = a + int_of_float (float_of_int (b - a) *. x) in
  let lerp_color c1 c2 x =
    let steps = 100 in
    let x =
      float_of_int (int_of_float (x *. float_of_int steps))
      /. float_of_int steps
    in
    let r1, g1, b1, a1 = c1 in
    let r2, g2, b2, a2 = c2 in
    (lerp r1 r2 x, lerp g1 g2 x, lerp b1 b2 x, lerp a1 a2 x)
  in
  let tuple_color (r, g, b, a) = Color.create r g b a in
  let variance = 10 in
  let calc_for_stripes x = x / 10 mod 10 in
  let calc_for_specals x =
    let xf = float_of_int x in
    int_of_float (float_of_int variance *. Float.sin xf)
  in
  let calc_for_slights x =
    let xf = float_of_int x in
    let ans = Float.sin (xf *. 2.0) +. Float.sin (xf *. Float.pi) in
    int_of_float (float_of_int variance *. ans)
  in
  let calc_for_noise x =
    let fx = float_of_int x in
    let rand = Noise.get fx in
    int_of_float @@ (rand *. float_of_int variance)
  in
  let shake_color seed (r, g, b, a) =
    let seed = seed + r + g + b in
    let calc_random x =
      if r mod 2 = 0 then calc_for_noise x
      else if r mod 3 = 0 then calc_for_specals x
      else if r mod 5 = 0 then calc_for_slights x
      else calc_for_stripes x
    in
    let bound_rand upper seed value =
      min (max (value + calc_random seed - (variance / 2)) 0) upper
    in
  ...
  if Moonshot.true_draw then
    ...
    |> List.map (fun x ->
           List.init (2 * r) (fun i -> (x, y - r + (i * size))))
    |> List.flatten
    |> List.filter (fun (px, py) ->
           square (py - y + (size / 2)) + square (px - x + (size / 2))
           < int_of_float (rf *. rf))
    |> List.map (fun (px, py) ->
           let d2 =
             square (py - y + (size / 2)) + square (px - x + (size / 2))
           ...
           let p = d /. rf in
           let shake_seed =
             (px + (py * 2 * r) + ((x + (y * 10000)) * 1000000)) * 10
           in
           (px, py, shake_color shake_seed @@ lerp_color ci co p))
    |> List.iter (fun (x, y, c) ->
           draw_rectangle x y size size (tuple_color c)) ;
...
let draw_planet p =
  ...
  let color =
    ...
    else color_create 200 200 200 255
    (* lightgray *)
  in
  let r = p.radius in
  ...
  let color_inner, color_outer =
    ...
    | Body.Normal when density < 0.0 ->
        (color_create 0 0 0 0 (* blank *), color)
    | Body.Normal -> (color, color)
  in
  draw_blocky_circle px py pr color_inner color_outer

let draw_powerup static powerup =
  let {Powerup.power; pos} = powerup in
  let x, y = sofwv pos in
  let size = float_of_int @@ sofw @@ Powerup.draw_size power in
  let gfx = match power with Powerup.Fireblast -> Color.red in
  let b = {Body.mass= 1.0; radius= size; pos} in
  let open Body in
  let fx, fy =
    Update.gravity_from_many b
      (List.map (fun x -> ignore x.surface ; x.body) static)
  in
...
let level_textbox =
  draw_text_box (Moonshot.font_size * 3)
    ((Moonshot.screen_width - box_width) / 2)
    0 box_width Color.raywhite
    (Color.create 49 49 49 100)

let draw_playing_starfield stars px py =
  let star_backoff = 5.0 in
  let px, py = (px /. star_backoff, py /. star_backoff) in
  Starfield.draw stars px py
    (Moonshot.sofw (1.0 /. star_backoff))
    Color.raywhite

let draw_playing model =
  let { Moonshot.Model.bullets= movables
      ; static
      ; fading
      ; player=
          { animation_state= ani
  ...
  begin_drawing () ;
  clear_background
    (if Moonshot.true_draw then Color.black else Color.raywhite) ;
  ...
  List.iter
    (fun x ->
      ...
      let text =
        ...
        | Normalblast -> Images.BulletFlying
      in
      draw_texture
        (Images.get_animation text)
        x.moving.body.pos 0.0
        (2.0 *. x.moving.body.radius) ;
      if Moonshot.debug_draw then draw_body Color.gray x.moving.body)
    movables ;
  (* Draw powerups *)
  ...
  let draw_hearts i =
    ...
    List.iter
      (fun x ->
        ...
        draw_texture_hud
          (Images.get Images.HeartFull)
          (Vector2.create fx fy)
          (3 * int_of_float heart_radius) ;
        if Moonshot.debug_draw then
          draw_circle x heart_offset heart_radius Color.red)
      xs
  in
  let draw_half_heart x =
    let fx, fy = (float_of_int x, float_of_int heart_offset) in
    draw_texture_hud
      (Images.get Images.HeartHalf)
      (Vector2.create fx fy)
      (3 * int_of_float heart_radius) ;
    if Moonshot.debug_draw then
      draw_circle x heart_offset (heart_radius /. 2.0) Color.red
  in
  ...
  draw_text
    (string_of_float trunc_time)
    (Moonshot.screen_width - (fsize * 5))
    fsize fsize Color.gold ;
  if runtime < 5.0 then level_textbox model.start_text ;
  end_drawing () ;
  Model.Playing model
...
let line_drawer font cx sy color =
  ...
  let draw_line ?(size = font) ?(centered = true) text =
    (if centered then draw_centered_text else draw_text)
      text cx (sy + !offset) size color ;
    offset := !offset + size
  in
  draw_line
...
let draw_paused model =
  ...
  let ld =
    line_drawer (Moonshot.font_size * 3)
      (Moonshot.screen_width / 2)
      (Moonshot.screen_height / 4)
      Color.raywhite
  in
...
let draw_menu_starfield _ =
  Starfield.draw menu_starfield
    (10.0 *. get_time ())
    (get_time ()) (Moonshot.sofw 1.0) Color.raywhite
...
let draw_stars fcolor scolor has_health_star has_shot_star has_time_star =
  ...
  let draw_star x =
    ...
    draw_texture_hud
      (Images.get_animation Images.StarShine)
      (Vector2.create xf yf) (2 * r)
  in
...
let draw_levelend model =
  ...
  draw_menu_starfield () ;
  let msg =
    match model.reason with
    | Victory -> "Victory!!"
    | DriftedAway -> "Lost in space :("
    | Died -> "You Died :("
  in
  let has_time_star =
    model.reason = Victory && model.star_reqs.time >= model.runtime
  in
  let has_shot_star =
    model.reason = Victory && model.star_reqs.shots >= model.shots_taken
  in
  let has_health_star =
    model.reason = Victory && model.star_reqs.health <= model.health
  in
  ...
  let ld =
    line_drawer (Moonshot.font_size * 2) cx
      (Moonshot.screen_height / 4)
      fcolor
  in
  ...
  ld
    (Printf.sprintf "Your longest shot stayed in orbit for %.2f seconds"
       model.longest_bullet) ;
...
let draw_stats p =
  ...
  let ld =
    line_drawer
      (ifmul Moonshot.font_size 1.8)
      (Moonshot.screen_width / 2)
      (Moonshot.screen_height / 4)
      f_color
  in
  ld ~size:(Moonshot.font_size * 4) "Stats" ;
  if Option.is_none p then ld "Press [space] to return to main menu"
  else ld "Press [space] to go up a level" ;
  ...
  | Some (p, None) ->
      ld
        ( "Press [1-9,0,-,=] to select a level in world "
        ^ string_of_int (p / 100)
        ^ " details" )
  | Some (w, Some p) ->
      ...
      draw_centered_text
        (Printf.sprintf "Selected level %d of world %d" p (w / 100))
        cx y font f_color ;
      let h, s, t = Savedata.level (w - 100 + p) in
      draw_stars f_color Color.gold h s t ) ;
  end_drawing () ; Model.StatsScreen p
...
let draw_worldselect _ =
  ...
  let ldl =
    line_drawer
      (ifmul Moonshot.font_size 2.0)
      (5 * Moonshot.screen_width / 16)
      lld_y f_color
  in
  ...
  List.iter
    (fun level ->
      ldl ~centered:false
        (Printf.sprintf "[%s] World %d: %s"
           (List.nth keys (level - 1))
           level
...
let draw_levelselect w =
  ...
  let ld =
    line_drawer
      ...
      f_color
  in
  ld ~size:(Moonshot.font_size * 4)
    ("World " ^ string_of_int (w / 100) ^ " Level Select") ;
  ...
  List.iter
    (fun level ->
      ...
      (if level mod 2 = 1 then ldl else ldr)
        ~centered:false
        (Printf.sprintf "[%s] Level %d (%s%s%s)"
           (List.nth keys (level - 1))
           level (star s1) (star s2) (star s3)))
